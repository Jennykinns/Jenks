...
class TabPaint(BaseTab):
    ...
    def updateColorDisplaySettings(self):
        ...
        for displayNode in self.colorDisplayNodes:
            if cmds.objExists(displayNode):
                cmds.setAttr(displayNode+".influenceWeightsMode",influenceWeightsMode)
    
    def paintCtxSetupProcedure(self,toolContext):
        # add and configure display nodes
        cmds.ngSkinLayer(colorDisplayNode=1)
        self.colorDisplayNodes = cmds.ls(type='ngSkinLayerDisplay')
        if self.colorDisplayNodes is None:
            self.colorDisplayNodes = []
