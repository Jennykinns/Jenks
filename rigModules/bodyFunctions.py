...
class armModule:
    ...
    def create(self, options=defaultBodyOptions.arm, autoOrient=False):
        ...
        if options['IK']:
            armMechGrp = utils.newNode('group', name='{}arm'.format(extraName),
        ...
        if options['IK'] and options['FK']:
            ...
            cmds.parent(ikJnts[0], fkJnts[0], armMechGrp.name)
        ...
        if options['IK']:
            ...
            self.clavIKCtrl = ctrlFn.ctrl(name='{}clavicleIK'.format(extraName), side=self.side,
                                        guide=ikJnts[0], skipNum=True, parent=ikCtrlGrp.name)
            ...
            self.handIKCtrl = ctrlFn.ctrl(name='{}handIK'.format(extraName), side=self.side,
                                            guide=ikJnts[-1], skipNum=True, parent=ikCtrlGrp.name)
        ...
        if options['FK']:
            ...
            self.clavFKCtrl = ctrlFn.ctrl(name='{}clavicleFK'.format(extraName), side=self.side,
                                          guide=fkJnts[0], skipNum=True, parent=fkCtrlGrp.name)
            self.clavFKCtrl.modifyShape(color=col['col2'], shape='pin')
            self.clavFKCtrl.lockAttr(attr=['t', 's'])
            self.clavFKCtrl.constrain(fkJnts[0], typ='orient')
            ...
            self.shoulderFKCtrl.constrain(fkJnts[1], typ='orient')
            ...
            self.elbowFKCtrl.constrain(fkJnts[2], typ='orient')
            ...
            self.wristFKCtrl.constrain(fkJnts[3], typ='orient')
